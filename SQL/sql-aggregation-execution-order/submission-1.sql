...
SELECT country, year, SUM(gold) + SUM (silver) + SUM(bronze) AS total_medals FROM olympic_medals WHERE category != 'Gymnastics' GROUP BY country, year HAVING SUM(gold) + SUM (silver) + SUM(bronze) > 20 ORDER BY total_medals DESC LIMIT 5;
